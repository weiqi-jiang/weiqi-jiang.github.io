...
在x发生的条件下，y带来的新的熵称为x的条件熵H(Y\|X) 衡量已知随机变量x的情况，y的不确定性$H(X|Y)=H(X,Y)-H(X)$
...
**这里有一个很重要的问题，为什么不为0中心，就导致收敛慢？**在梯度更新时需要对$sigmoid(w*x)$对$w$求偏导结果为$sigmoid(w*x)(1-sigmoid(w*x))x$， 除x项外，其他都是常数，且恒大于0，从第二层起，神经元经过激活函数的输出都是正值，所以后续在做BP的时候，梯度恒为正。

**ReLU**

![relu](/assets/img/deeplearning/one-stop/relu.png)

对比sigmoid，ReLU函数主要变化是
